...
export const useSnackbar = () => {
    ...
    const [position, setPosition] = useState<Position>({ vertical: 'bottom', horizontal: 'center' });
    ...
    const OpenSnackbar = (message: string, status?: Status, position?: Position) => {
        if (status) {
            setStatus(status);
        } else if (position) {
    ...
    const SetSnackbarPosition = ({ vertical = 'bottom', horizontal = 'center' }: Position) => {
