...
function sortBestMatch(
  mainString: TTimezone = '',
  targetObjects: ICityData[] = [],
  path: string
): IMatchingLocation[] {
  ...
  for (let i = 0; i < targetObjects.length; i += 1) {
    const element: ICityData = targetObjects[i];
    let currentTargetString;
    if (typeof element === 'object' && path) {
      currentTargetString = element[path as keyof ICityData];
    } else {
      currentTargetString = element;
    }

    if (!currentTargetString || typeof currentTargetString !== 'string')
      return [];
